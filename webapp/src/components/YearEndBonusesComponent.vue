<template>
                                        ...
                                        <div style="display:none">
                                            ...
                                            <div dy-model="number">{{bonus.Year}}</div>
                                            <div dy-model="string">{{bonus.Lang}}</div>
                                        </div>
                                        <h2 class="title-header">玉山人園地 - 年終查詢</h2>
                                        <div class="contents">
                                            <p>
                                                現況服務部門:
                                                <span id="MainContent_lblEMP_Department" v-on:click="Aggr_Post('hr', 'EFP_GetBonusAndSalary', 'url:CompanyCode:CompanyCode', 'url:EmpID:EmpID', 'url:Year:Year', 'url:Lang:Lang')" dy-model="string">{{bonus.Department}}</span>
                                            </p>
                                            <div dy-model="string">{{bonus.Selected}}</div>
                                            <p>
                                                發薪單位：
                                                <select v-model="bonus.Selected" style="width:300px;" dy-watch="bonus.Selected, Aggr_Post('hr', 'EFP_GetBonusAndSalary', 'url:CompanyCode:CompanyCode', 'url:EmpID:EmpID', 'url:Year:Year', 'url:Lang:Lang')" dy-selectinitial="Aggr_Post('hr', 'EFP_GetBonusCompany','url:CompanyCode:CompanyCode', 'url:EmpID:EmpID', 'url:Year:Year', 'url:Lang:Lang'))">
                                                    <option dy-model="object[]" v-for="item in bonus.CompanyInfo" v-bind:value="item.CompanyTypeCode" v-bind:key="item.CompanyTypeCode" dy-vfor-model="number">{{item.CompanyTypeName}}</option>
                                                </select>
                                            </p>
                                                ...
                                                <div id="MainContent_panelBonus">
                                                    ...
                                                    <span id="MainContent_lbl_BounsesContent"></span><table class="supplycontent-table-data">
                                                        <tbody>
                                                            ...
                                                            <tr dy-model="object[]" v-for="item in bonus.ERDD">
                                                                <td>{{item.ERName}}</td>
                                                                <td dy-vfor-model="number">{{item.ERSalary}}</td>
                                                                <td>{{item.DDName}}</td>
                                                                <td dy-vfor-model="number">{{item.DDSalary}}</td>
                                                            </tr>
                                                            <tr>
                                                                <th class="colored-bg">應發數</th>
                                                                <th>
                                                                    <span class="th_r">
                                                                        <span id="MainContent_lblPaySum" style="color: black; font-weight: 700" dy-model="number">{{bonus.ERTotal}}</span>
                                                                    </span>
                                                                </th>
                                                                <th class="colored-bg">應扣數</th>
                                                                <th>
                                                                    <span class="th_r">
                                                                        <span id="MainContent_lblWithHoldSum" style="color: black; font-weight: 700" dy-model="number">{{bonus.DDTotal}}</span>
                                                                    </span>
                                                                </th>
                                                            </tr>
                                                            <tr class="colored-bg">
                                                            ...
                                                            <tr>
                                                                <th colspan="4">
                                                                    <span id="MainContent_lblSum" style="color: black; font-weight: 700" dy-model="number">{{bonus.Total}}</span>
                                                                </th>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                    ...
                                                    <span id="MainContent_lb_BonusRemark"></span><table class="table-notice">
                                                        <tbody>
                                                            <tr dy-model="string[]" v-for="item in bonus.Remark"><th style="color:#99cc66;">※</th><td>{{item}}</td></tr>
                                                        </tbody>
                                                    </table>
                                                </div>
                                            </div>
                                            ...
                                            <div id="sShowYearSalary">
                                                <div id="ShowYearS">
                                                    ...
                                                    <span id="MainContent_lbSalarytable">
                                                        <table class="supplycontent-table-data">
                                                            <tbody>
                                                                <tr dy-model="object[]" v-for="item in bonus.AnnualSalary"><th class="colored-bg">{{item.SalaryItemCategoryIIIName}}</th><th>{{item.SalaryAmt}}</th></tr>
                                                            </tbody>
                                                        </table>
                                                    </span>
                                                </div>
