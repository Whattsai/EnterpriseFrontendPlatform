<template>
    ...
    <div class="contents">
        ...
        <p>
            ...
            <select v-model="bonusInfo.Selected" style="width:300px;" dyweb-watch="bonusInfo.Selected,Aggr_Post('C','Title')" dyweb-selectinitial="Aggr_Post('selectinitial','Title'))">
                <option dyweb-model="object[]" v-for="item in bonusInfo.Companies" :value="item.ID" :key="item.ID" dyweb-model-vfor="Companies, string">{{item.ID}}</option>
            </select>
        </p>
                ...
                <span id="MainContent_lbl_BounsesContent"></span><table class="supplycontent-table-data">
                    <tbody>
                        ...
                        <tr dyweb-model="object[]" v-for="item in bonusInfo.ER">
                            <td dyweb-model-vfor="ER, string">{{item.Description}}</td>
                            <td dyweb-model-vfor="ER, number">{{item.Amount}}</td>
                            <td>獎金補充保費</td>
                            <td>1,250</td>
                        </tr>
                        <tr>
    ...
    <span id="MainContent_lb_BonusRemark"></span>
